...
def classify_shot_role(shot: Shot) -> ShotRole:
    ...
    # Action: high motion
    scores[ShotRole.ACTION] += q.motion_intensity * 0.6
    if q.motion_intensity > 0.4:
        scores[ShotRole.ACTION] += 0.2
    scores[ShotRole.ACTION] += (1.0 - q.stability) * 0.2  # Unstable = action

    # Reaction: face-heavy, lower motion
    scores[ShotRole.REACTION] += q.face_score * 0.6
    if q.face_score > 0.3:
        scores[ShotRole.REACTION] += 0.2
    scores[ShotRole.REACTION] += (1.0 - q.motion_intensity) * 0.2
